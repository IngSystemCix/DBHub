...
/**
 * Implementation of the DatabaseConnector interface for Oracle databases.
 * Provides methods to connect, disconnect, check connection status,
 * and handle error messages specific to Oracle.
 */
public class OracleConnector implements DatabaseConnector, DatabaseQuerys {
    ...
    public boolean connect(String server, String port, String database, String user, String password) {
        String JDBC_URL = String.format("jdbc:oracle:thin:@%s:%s:%s", server, port, database);
        try {
            connection = DriverManager.getConnection(JDBC_URL, user, password);
            return true;
        } catch (SQLException e) {
            setErrorMessage("Error connecting to Oracle: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean disconnect() {
        if (connection != null) {
            try {
                ...
                setErrorMessage("Error disconnecting from Oracle: " + e.getMessage());
                return false;
            }
        }
        setErrorMessage("No Oracle connection to close.");
        return false;
    }

    /**
     * Checks if there is an active connection to the Oracle database.
     *
    ...
    public boolean isConnected() {
        try {
            ...
            setErrorMessage("Error checking Oracle connection status: " + e.getMessage());
            return false;
        }
    }
